...
abstract: "Based on the recent social media advertisement trend of financial courses, we investigate the likelihood of having an annual income higher than or equal to $100,000CAD given young adult males who are self employed and work on average less than 30-40 hours a week. To accomplish this we fitted an elastic net logistic regression model using the Canadian 2016 General Social Survey and concluded on the unlikeliness of this to happen."
---
...
there are many books that make such promises which where published 
...
courses claim that it is possible to have a high income with working 4-20 hours
...
young adult males, and hence the reason why focus on this gender for our
...
to go along the research question and the opening discussion of the problem.
Therefore, our model choice, which is perhaps the most used and
suited for binary classification, is the logistic regression model. We assume the
reader has some familiarity with this model.
...
here we assume $\beta=(\beta_1, \dots, \beta_p)^T$ is a vector of appropriate size for left multiplication
...
\[\ell_{elnet}(\beta_0, \beta)=\left[\sum_{i=1}^Ny_i(\beta_0+\beta^T\mathbf{x}_i)-\log(1 + exp\{\beta_0+\beta^T\mathbf{x}_i\})\right] - \left[\lambda_1\sum_{j=1}^p|\beta_j| + \lambda\sum_{j=1}^p\beta_j^2\right]\]
in the equation above we assume that $\beta=(\beta_1,\dots, \beta_p)$. Also in this
new equation, the value $\lambda_1\sum_{j=1}^p|\beta_j|$ is the $L_1$ regularization penalty, and
similarly $\lambda\sum_{j=1}^p\beta_j^2$ the $L_2$ regularization penalty.
...
investments is higher. It also no surprise the positive relationship between number of hours 
and income. In a country like Canada where jobs are usually compensated based on 
hourly work this is what's to be expected.

What is interesting, however, is the little effect of the self employment
...
alternative is to fit a decision tree model.
...
Finally, as per the introduction, we answer the research question. Consider
a young adult male in the age group 25-34 who is self 
employed and who works less than the average of 30-40. Our model then estimates 
...
enter and click on "General Social Survey". Pick a year and click on "data", then
